...
def plot_results(country, target_subject, start_year, end_year, truth_file):
    ...
    try:
        estimates_start_after = truth_df["Estimates Start After"].values[0]
    ...
    for y in truth_target_series.index:
        if y > estimates_start_after:
            truth_target_series.loc[y] = np.nan
    ...
    organism_series = pd.Series(rpch_values_truth, index=years)
    ...
    # 1️⃣ 误差柱状图
    error_with_plot = error_with[valid_indices]
    error_without_plot = error_without[valid_indices]
    ...
    plt.title(f'Prediction Errors Comparison for {target_subject}_PCH ({country})')
    plt.legend()
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{target_subject}_PCH_error_compare.png'), dpi=300)
    plt.close()
    ...
    plt.ylabel('RMSE')
    plt.title(f'Overall RMSE Comparison for {target_subject}_PCH Predictions ({country})')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{target_subject}_PCH_rmse_compare.png'), dpi=300)
    plt.close()
    ...
    output_path = f'figure/{country}/{country}_{target_subject}_PCH_pred_vs_true_compare.csv'
    df_out.to_csv(output_path, index=False)
    print(f"Saved CSV to {output_path}")

    # 4️⃣ 累积 RMSE
    rmse_with_all = [np.sqrt(np.nanmean(error_with[:idx + 1] ** 2)) for idx in range(len(years))]
    rmse_without_all = [np.sqrt(np.nanmean(error_without[:idx + 1] ** 2)) for idx in range(len(years))]
    rmse_organism_all = [np.sqrt(np.nanmean(error_weo[:idx + 1] ** 2)) for idx in range(len(years))]
    ...
    plt.tick_params(axis='both', which='major', labelsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{target_subject}_PCH_rmse_all_years_compare.png'), dpi=400)
    plt.close()
