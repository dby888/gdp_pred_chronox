...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        if not self.is_decoder:
            # self.iib_linear_emb = nn.Linear(config.d_model, config.d_model)
            # self.iib_linear_cov = nn.Linear(1, config.d_model)  # NFCI 1 → d_model
            # self.iib_ffn = nn.Linear(2 * config.d_model, config.d_model)  # concat 后 Linear mapping
            self.iib_linear_emb = nn.Linear(self.config.d_model, 256)
            self.iib_linear_cov = None
    ...
    def forward(
        ...
        if covariates is not None and self.iib_linear_cov is None:
            F = covariates.shape[-1]
            self.iib_linear_cov = nn.Linear(F, 256).to(covariates.device)
        # Model parallel
        ...
        # 新增
        if covariates is not None and not self.is_decoder:
            cov_embeds = self.iib_linear_cov(covariates)  # (batch, seq_len, d_model)
            emb_embeds = self.iib_linear_emb(inputs_embeds)  # (batch, seq_len, d_model)
            combined = torch.cat([emb_embeds, cov_embeds], dim=-1)  # → (batch, seq_len, 2*d_model)
            inputs_embeds = inputs_embeds + self.iib_ffn(combined)  # iib_ffn 变成 Linear(2*d_model → d_model)
